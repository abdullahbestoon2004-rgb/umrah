...
import 'agency_trip_hub_screen.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return DashboardScaffold(
      ...
      trailing: company.isVerified
          ...
          : null,
      onRefresh: () => context.read<AppProvider>().loadAgencyOffers(),
      slivers: [
        if (offers.isEmpty)
          SliverFillRemaining(
            hasScrollBody: false,
...
class PackageCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => AgencyTripHubScreen(offer: offer)),
      ),
      child: Container(
        decoration: BoxDecoration(
                        ...
                        child: Wrap(
                          ...
                          children: [
                            ...
                            if (offer.departureDate != null)
                              InfoChip(
                                label:
                                    '${offer.departureDate!.day}/${offer.departureDate!.month}/${offer.departureDate!.year}',
                              ),
                            if (offer.remainingSeats != null)
                              InfoChip(
                                label: t.offerCapacityRemaining(
                  ...
                  if (_isAgencyOwned &&
                    ...
                    SizedBox(
                      ...
                      child: OutlinedButton.icon(
                        onPressed: () async {
                          final messenger = ScaffoldMessenger.of(context);
                          final error = await provider.pauseOffer(offer.id);
                          messenger.showSnackBar(
                            appSnack(
                              error ?? t.workflowPausedSnack,
                              isError: error != null,
                            ),
                          );
                        },
                        icon: const Icon(Icons.pause_circle_outline_rounded),
                        label: Text(t.workflowPauseTrip),
                      ),
                    ),
                  ],
                ],
