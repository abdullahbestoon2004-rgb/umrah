...
class _NotificationsScreenState extends State<NotificationsScreen> {
  /// Opening this screen counts as seeing every notification: the unread
  /// badge clears immediately and stays cleared. Entries that were unread on
  /// arrival keep their "new" styling for the rest of this visit.
  ...
  void initState() {
    ...
    _newIds = provider.notifications
        .where((n) => !n.read)
        .map((n) => n.id)
        .toSet();
    if (_newIds.isNotEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) context.read<AppProvider>().markAllNotificationsRead();
      });
    }
  }
                      ...
                      itemBuilder: (_) => [
                        PopupMenuItem(
                          ...
                          child: Text(
                            ...
                            style: AppTheme.sans(13, color: AppColors.errorRed),
                          ),
                        ),
                      ],
                    ),
                      ...
                      itemBuilder: (context, i) {
                        ...
                        return Dismissible(
                          ...
                          child: _NotificationCard(
                            item: item,
                            isNew: !item.read || _newIds.contains(item.id),
                          ),
                        );
                      },
                    ),
...
class _NotificationCard extends StatelessWidget {
  ...
  IconData get _icon {
    switch (item.type) {
      ...
      case NotificationType.companyReview:
        return Icons.domain_verification_rounded;
      case NotificationType.packageReview:
  ...
  String _title(AppLocalizations t) {
    switch (item.type) {
      case NotificationType.welcome:
        return t.notifWelcomeTitle;
      case NotificationType.promo:
        return t.notifPromoTitle;
      case NotificationType.tripReminder:
        return t.notifTripReminderTitle;
      case NotificationType.bookingRequested:
        return t.notifBookingRequestedTitle;
      case NotificationType.bookingConfirmed:
        return t.notifBookingConfirmedTitle;
      case NotificationType.bookingCancelled:
        return t.notifBookingCancelledTitle;
      case NotificationType.companyReview:
        return t.workflowCompanyReviewTitle;
      case NotificationType.packageReview:
        return t.workflowPackageReviewTitle;
    }
  }
  ...
  String _body(AppLocalizations t) {
    ...
    switch (item.type) {
      case NotificationType.welcome:
        return t.notifWelcomeBody;
      case NotificationType.promo:
        return t.notifPromoBody;
      case NotificationType.tripReminder:
        return t.notifTripReminderBody(arg);
      case NotificationType.bookingRequested:
        return t.notifBookingRequestedBody(arg);
      case NotificationType.bookingConfirmed:
        return t.notifBookingConfirmedBody(arg);
      case NotificationType.bookingCancelled:
        return t.notifBookingCancelledBody(arg);
      case NotificationType.companyReview:
        return t.workflowCompanyReviewBody(arg);
      case NotificationType.packageReview:
        return t.workflowPackageReviewBody(arg);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return InteractiveScale(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isNew ? const Color(0xFFEFF6F1) : AppColors.surface,
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: isNew
